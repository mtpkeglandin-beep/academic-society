...
export default function AnalyticsPage() {
  ...
  const stats = useMemo(() => {
    ...
    const filteredEvents = events.filter(ev => isWithinInterval(new Date(ev.start_date), { start: startDate, end: now }));
    const counts: Record<string, number> = {};
    filteredEvents.forEach(ev => { ev.attendees?.forEach(name => { counts[name] = (counts[name] || 0) + 1; }); });
    ...
    <div className="min-h-screen bg-[#f8fafc] pb-24 lg:pb-12">
      ...
      <div className="relative max-w-6xl mx-auto px-6 pt-8 lg:pt-12 space-y-8 lg:space-y-12">
        <div className="flex justify-between items-center text-white">
          <Link href="/" className="inline-flex items-center gap-2 font-bold opacity-80 hover:opacity-100 transition-all">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
          <div className="text-white space-y-2">
            ...
            <p className="font-medium text-blue-100 opacity-80">영업 부문별 실시간 학회 참석 실적입니다.</p>
          </div>
          
          <div className="flex flex-wrap gap-3 bg-white/10 backdrop-blur-md p-3 rounded-[2rem] border border-white/10">
            <Select value={period} onValueChange={setPeriod}>
              <SelectTrigger className="w-[140px] lg:w-[160px] font-black bg-white/90 border-none h-12 rounded-2xl"><SelectValue /></SelectTrigger>
              <SelectContent>
            ...
            <Select value={selectedAffiliation} onValueChange={setSelectedAffiliation}>
              <SelectTrigger className="w-[160px] lg:w-[200px] font-black bg-white/90 border-none h-12 rounded-2xl"><SelectValue /></SelectTrigger>
              <SelectContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* 하이라이트: 1등 MR 카드 */}
          <Card className="lg:col-span-1 border-none shadow-[0_30px_60px_rgba(37,99,235,0.2)] bg-white rounded-[3rem] p-8 lg:p-10 flex flex-col justify-between min-h-[400px]">
            <div className="space-y-6">
              <div className="h-16 w-16 bg-orange-100 rounded-3xl flex items-center justify-center">
                <Flame className="h-8 w-8 text-orange-500" />
              </div>
              <div className="space-y-1">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Active Leader</p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between px-2">
                <span className="text-sm font-bold text-slate-400">학회 참석 횟수</span>
          ...
          <Card className="lg:col-span-2 border-none shadow-xl bg-white rounded-[3rem] overflow-hidden">
            <CardHeader className="bg-slate-50/50 border-b p-8">
              <CardTitle className="text-sm font-black flex items-center gap-3 text-slate-500 uppercase tracking-[0.2em]">
                <Target className="h-5 w-5 text-blue-600" /> Member Ranking
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <div className="divide-y divide-slate-50 max-h-[600px] overflow-y-auto scrollbar-hide">
                {stats.map((s, i) => (
                  <div key={s.name} className="flex items-center justify-between p-6 lg:p-8 hover:bg-slate-50/50 transition-all group">
