...
import { isWithinInterval, parseISO, getYear, getMonth } from 'date-fns';
...
export default function HomePage() {
  ...
  const [filterOrganizer, setFilterOrganizer] = useState<string>('all'); // 👈 주관기관 필터 추가
  ...
  // 🏛️ 등록된 데이터에서 유니크한 주관기관 목록 추출
  const organizers = useMemo(() => {
    const list = events.map(ev => ev.organizer.trim());
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter(ev => {
      ...
      const matchOrganizer = filterOrganizer === 'all' || ev.organizer === filterOrganizer; // 👈 주관기관 조건 추가

      return matchYear && matchMonth && matchProduct && matchOrganizer;
    });
  }, [events, selectedDate, activeTab, filterYear, filterMonth, filterProduct, filterOrganizer]);
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] pb-24 lg:pb-12 font-sans text-slate-900">
      <header className="sticky top-0 z-50 w-full border-b border-white/40 bg-white/70 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 h-16 lg:h-20 flex justify-between items-center text-slate-900">
          <Link href="/" className="flex flex-col group">
            <span className="text-xl lg:text-3xl font-black tracking-tighter italic group-hover:text-blue-600 transition-all">학회 일정 공유</span>
            <span className="text-[9px] font-black text-blue-600 uppercase tracking-[0.3em] leading-none">Scheduler</span>
          </Link>
          <nav className="hidden md:flex items-center gap-1 bg-slate-200/50 p-1.5 rounded-2xl">
            ...
            <Link href="/analytics" className="px-6 py-2 rounded-xl text-sm font-black text-slate-500 hover:text-slate-900 transition-all">참석자 분석</Link>
          </nav>
      ...
      <main className="max-w-7xl mx-auto px-4 lg:px-8 py-8 lg:py-12 space-y-10 text-slate-900">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {(selectedDate || filterProduct !== 'all' || filterOrganizer !== 'all') && (
                  <Button 
                    variant="ghost" 
                    size="sm" 
          ...
          {activeTab === 'table' && !selectedDate && (
            <div className="flex flex-wrap gap-3 bg-white/50 backdrop-blur-sm p-4 rounded-[2rem] border border-white/40 shadow-sm animate-in fade-in slide-in-from-left-4 duration-500">
              ...
              <Select value={filterYear} onValueChange={setFilterYear}>
                <SelectTrigger className="w-[110px] bg-white border-none rounded-xl font-bold shadow-sm h-11 text-slate-900"><SelectValue placeholder="연도" /></SelectTrigger>
              ...
              <Select value={filterMonth} onValueChange={setFilterMonth}>
                <SelectTrigger className="w-[100px] bg-white border-none rounded-xl font-bold shadow-sm h-11 text-slate-900"><SelectValue placeholder="월" /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={filterProduct} onValueChange={setFilterProduct}>
                <SelectTrigger className="w-[130px] bg-white border-none rounded-xl font-bold shadow-sm h-11 text-slate-900"><SelectValue placeholder="품목" /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={filterOrganizer} onValueChange={setFilterOrganizer}>
                <SelectTrigger className="w-[180px] bg-white border-none rounded-xl font-bold shadow-sm h-11 text-slate-900"><div className="flex items-center gap-2"><Building2 className="h-3.5 w-3.5 text-blue-500" /><SelectValue placeholder="주관기관" /></div></SelectTrigger>
                <SelectContent className="max-h-[300px]">
                  <SelectItem value="all">전체 주관기관</SelectItem>
                  {organizers.map(org => (
