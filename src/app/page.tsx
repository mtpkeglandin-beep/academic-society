...
export default function HomePage() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter(ev => {
      ...
      } catch { return ev.start_date === selectedDate; }
    });
  }, [events, selectedDate]);
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] pb-24 lg:pb-12 font-sans text-slate-900">
      <header className="sticky top-0 z-50 w-full border-b border-white/40 bg-white/70 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 h-16 lg:h-20 flex justify-between items-center text-slate-900">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-slate-200/50 p-1.5 rounded-2xl">
            <Link href="/" className="px-6 py-2 rounded-xl text-sm font-black bg-white shadow-md text-blue-600">대시보드</Link>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => setIsModalOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-black rounded-2xl h-10 lg:h-14 px-4 lg:px-8 shadow-xl shadow-blue-100 active:scale-95 flex gap-2">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8 lg:space-y-12">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            <div className="space-y-2 text-slate-900">
              ...
              <div className="flex items-center gap-2">
                <p className="text-sm lg:text-lg text-slate-500 font-bold">{selectedDate ? <span className="text-blue-600 bg-blue-50 px-4 py-1.5 rounded-full italic font-black animate-in zoom-in">📅 {selectedDate} 일정 확인 중</span> : '영업부 통합 학회 지원 현황입니다.'}</p>
            ...
            <TabsList className="grid grid-cols-2 w-full lg:w-[340px] bg-white p-1.5 rounded-[1.5rem] h-14 shadow-xl border border-slate-200">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 h-24 bg-white/90 backdrop-blur-xl border-t flex items-center justify-around px-8 z-50">
        <Link href="/" className="flex flex-col items-center gap-1.5 transition-all"><div className="p-3 rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-100"><LayoutDashboard className="h-6 w-6" /></div><span className="text-[10px] font-black uppercase text-blue-600 tracking-widest">Home</span></Link>
        <Link href="/analytics" className="flex flex-col items-center gap-1.5 transition-all"><div className="p-3 rounded-2xl text-slate-300"><BarChart3 className="h-6 w-6" /></div><span className="text-[10px] font-black uppercase text-slate-300 tracking-widest">Rank</span></Link>
      </div>
