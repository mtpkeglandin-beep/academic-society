  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const submissionData = { 
        ...
        booth_size: parseInt(formData.booth_size),
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden p-8 space-y-6 animate-in zoom-in duration-300 ring-1 ring-white/20">
        <div className="flex justify-between items-center text-slate-900 border-b pb-6">
          <div className="flex flex-col">
            <h2 className="text-3xl font-black italic tracking-tighter uppercase text-blue-600">Register</h2>
            <p className="text-[10px] font-black text-slate-400 tracking-widest uppercase">TPKR Schedule Hub</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 ml-2 uppercase">제품군</label>
            ...
            {/* ✅ 부스 사이즈 입력란 독립 확보 */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-black text-slate-400 ml-2 uppercase flex items-center gap-1"><LayoutGrid className="h-3 w-3" /> 부스 규모</label>
