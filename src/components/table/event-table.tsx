...
import { MapPin, Calendar, Users, UserPlus, XCircle, Trash2, LayoutGrid, CheckCircle2 } from 'lucide-react';
...
export default function EventTable({ events, onUpdate }: { events: Event[], onUpdate: () => void }) {
  ...
  const handleRemoveAttendee = async (eventId: string, currentAttendees: string[], targetName: string) => {
    if (confirm(`${targetName} 담당자를 삭제하시겠습니까?`)) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {events.map((ev) => {
        ...
        return (
          <div key={ev.id} className="group bg-white rounded-[3rem] p-10 shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-50 flex flex-col justify-between relative overflow-hidden ring-1 ring-slate-50 text-slate-900">
            ...
            <div className="space-y-8">
              <div className="flex justify-between items-start">
                ...
                {/* ✅ 부스 사이즈와 PM 참석 뱃지를 세로로 정렬하여 모두 표시 */}
                <div className="flex flex-col items-end gap-2">
                   ...
                   {/* ✅ PM 참석 표시 부활 (상단 통계와 일치됨) */}
                   {ev.pm_attend && (
                     <span className="flex items-center text-[10px] font-black text-white bg-slate-900 px-4 py-1.5 rounded-full shadow-lg">
                       <CheckCircle2 className="h-3.5 w-3.5 mr-1.5 text-blue-400" /> PM 참석
                     </span>
                   )}
                </div>
              </div>
                ...
                <div className="flex items-start gap-4 pt-4 border-t border-slate-50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {ev.attendees && ev.attendees.length > 0 ? ev.attendees.map(name => (
                      <button key={name} onClick={() => handleRemoveAttendee(ev.id, ev.attendees, name)} className="px-4 py-2 bg-slate-50 hover:bg-red-50 hover:text-red-600 rounded-xl text-sm font-black text-slate-700 border border-slate-100 shadow-sm flex items-center gap-2 group/btn transition-all uppercase">
                        {name}
                        <XCircle className="h-3 w-3 opacity-0 group-hover/btn:opacity-100 transition-opacity" />
                      </button>
                    )) : <span className="text-sm font-bold text-slate-300 italic py-1.5">담당자 미배정</span>}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-12 pt-8 border-t border-slate-100 flex gap-3">
              <input 
                placeholder="담당자 성함" 
                className="flex-1 bg-slate-50 border-none rounded-2xl px-5 text-sm font-black focus:ring-2 focus:ring-blue-500 h-14" 
