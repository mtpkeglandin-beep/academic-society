          ...
          <div key={ev.id} className="group bg-white rounded-[3rem] p-10 shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-50 flex flex-col justify-between relative overflow-hidden ring-1 ring-slate-50">
            ...
            <div className="space-y-8 text-slate-900">
              <div className="flex justify-between items-center">
                {/* 🏷️ 품목명: 글자 크기를 행사명 급(text-2xl)으로 대폭 확대 */}
                <span className={`px-6 py-2 rounded-2xl ${color.bg} ${color.text} text-2xl font-black uppercase tracking-tighter shadow-sm`}>
                  {ev.product}
                </span>
                {ev.pm_attend && (
                  <span className="flex items-center text-[11px] font-black text-blue-600 bg-blue-50 px-4 py-2 rounded-full ring-1 ring-blue-100">
                    ...
                    {ev.attendees && ev.attendees.length > 0 ? (
                      ev.attendees.map(name => (
                        <button 
                          ...
                          // 👥 참석자 이름: 가독성을 위해 text-sm으로 확대
                          className="px-3.5 py-1.5 bg-slate-50 hover:bg-red-50 hover:text-red-600 rounded-xl text-sm font-black text-slate-700 transition-all flex items-center gap-2 group/btn border border-slate-100 hover:border-red-100 shadow-sm"
                        >
                          {name}
