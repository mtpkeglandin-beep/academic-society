...
import { MapPin, Calendar, Users, UserPlus, CheckCircle2 } from 'lucide-react';
...
export default function EventTable({ events, onUpdate }: { events: Event[], onUpdate: () => void }) {
  ...
  const handleJoin = async (eventId: string, currentAttendees: string[]) => {
    ...
    if (!error) {
      alert(`${name} 담당자가 참석 명단에 추가되었습니다.`);
                  ...
                  <div className="flex flex-wrap gap-1">
                    {ev.attendees && ev.attendees.length > 0 ? (
                      ev.attendees.map(name => (
                        <span key={name} className="px-2 py-1 bg-slate-50 rounded-lg text-[10px] font-black text-slate-600">{name}</span>
                      ))
                    ) : (
